#!/usr/bin/env python3
"""
Plot phylogeny trees for sites requiring >= 2 independent mutations (recurrent/homoplastic).
Output: work/{palindrome}/recurrent/{palindrome_id}_{pos}_nmut{n}.pdf
"""
import os
import subprocess
...
def process_palindrome(palindrome: str) -> None:
    ...
    recurrent_dir = os.path.join(WORK_DIR, palindrome, "recurrent")
    ...
    for palindrome_id in stats:
        for pos in sorted(stats[palindrome_id].keys(), key=int):
            ...
            for genotype in nodes[root][4]:
                results: list = []
                traverse_tree(nodes[root], genotype, results)
            ...
            dot_file = os.path.join(recurrent_dir, f"temp_{pos}.dot")
            pdf_file = os.path.join(recurrent_dir, f"{palindrome_id}_{pos}_nmut{lowest_mutations}.pdf")

            with open(dot_file, "w") as out:
                out.write(f'digraph {{\nfontsize="10"\n')
                out.write(f'label="{palindrome_id} pos={pos} ({lowest_mutations} independent mutations)"\n')
    ...
    print(f"{palindrome}: {n_plotted} recurrent-mutation sites → {recurrent_dir}")
